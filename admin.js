...
router.get('/classes', authMiddleware, adminOnly, async (req, res) => {
  try { res.json(await Class.find().sort({ name: 1 })); }
...
router.post('/classes', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (await Class.findOne({ name })) return res.status(400).json({ error: `Class "${name}" already exists` });
    res.json({ message: 'Class added', class: await new Class({ name, department, year }).save() });
  } catch { res.status(500).json({ error: 'Server error' }); }
});
...
router.get('/teachers', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    res.json(teachers.map(t => {
      const { password, ...tWithoutPassword } = t;
      return { ...tWithoutPassword, isRegistered: !!password };
    }));
  }
  catch { res.status(500).json({ error: 'Server error' }); }
});
router.post('/teachers', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (!name || !email || !collegeId) return res.status(400).json({ error: 'Name, email and College ID required' });
    if (await User.findOne({ collegeId })) return res.status(400).json({ error: 'College ID already exists' });
    const teacher = await new User({ name, email, collegeId, password: '', role: 'teacher', department: department || '', classes: [] }).save();
...
router.put('/teachers/:id', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (classes && classes.length > 0) {
      ...
      if (invalid.length > 0) {
        return res.status(400).json({ error: `These classes don't exist: ${invalid.join(', ')}. Please add them first.` });
      }
    }
    const updated = await User.findByIdAndUpdate(req.params.id, { name, email, department, classes }, { returnDocument: 'after' }).select('-password');
...
// ── SUBJECTS — course code is GLOBALLY unique ──
router.get('/subjects', authMiddleware, adminOnly, async (req, res) => {
  try { res.json(await Subject.find().sort({ code: 1 })); }
  catch { res.status(500).json({ error: 'Server error' }); }
});
router.post('/subjects', authMiddleware, adminOnly, async (req, res) => {
  try {
    const { name, code, teacherId, class: cls } = req.body;
    if (!name || !code || !teacherId || !cls) return res.status(400).json({ error: 'All fields required' });

    const upperCode = code.trim().toUpperCase();

    // Check class exists
    if (!await Class.findOne({ name: cls })) {
      return res.status(400).json({ error: `Class "${cls}" does not exist. Add it in Classes section first.` });
    }
...
if (existing) {
  return res.status(400).json({ error: `Course code "${upperCode}" already exists.` });
    ...
    const subject = await new Subject({ name, code: upperCode, teacherId, class: cls }).save();
    await User.findByIdAndUpdate(teacher._id, { $addToSet: { classes: cls } });
...
router.put('/subjects/:id', authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const teacher = await User.findOne({ collegeId: teacherId, role: 'teacher' });
    if (!teacher) return res.status(400).json({ error: `Teacher "${teacherId}" not found` });
    const updated = await Subject.findByIdAndUpdate(req.params.id, { teacherId }, { returnDocument: 'after' });
...
router.get('/students', authMiddleware, adminOnly, async (req, res) => {
  try { res.json(await User.find({ role: 'student' }).select('-password').sort({ class: 1 })); }
  catch { res.status(500).json({ error: 'Server error' }); }
});
